<template>
	<div class="slider-beh" @mousemove="updateArrows()">
		<div v-swiper:mySwiper="swiperOption">
			<div class="swiper-wrapper">
				<div class="swiper-slide" v-for="slide in data.slideData">
					<nuxt-link to="/" class="slider__slide__link">
						...
						<div class="slider-beh__slide__info-area">
							...
							<h4 class="slider-beh__slide__title" :style="{ 'font-size': slide.beach ? '18px' : '20px' }">{{ slide.title }}</h4>
							<!-- <nuxt-link @click-prevent :to="slide.beachLink" class="slider-beh__slide__beach" v-if="slide.beach">{{ slide.beach }}</nuxt-link> -->
							<h6 class="slider-beh__slide__location" :style="{ 'font-size': slide.beach ? '10px' : '12px' }">{{ slide.location }}</h6>
							<!-- <nuxt-link :to="slide.priceLink" @click-prevent class="slider-beh__slide__price" :style="{ 'font-size': slide.beach ? '10px' : '12px' }" v-if="slide.price">от {{ slide.price }} ₽/сутки</nuxt-link> -->
						</div>
					</nuxt-link>
				</div>
			</div>
