<template>
	<button
    class="heart"
    @click.stop="updateHeart"
    @mouseenter="updateHeartHover()"
    @mouseleave="updateHeartHover()"
    v-if="data.showFavorite != undefined"
  >
    <svg class="svg-heart" width="12" height="11" viewBox="0 0 23 22" xmlns="http://www.w3.org/2000/svg">
      <path
        :class="[
          {lovely: isFav || favoriteHover && !isFav},
          {unknown: !isFav && !favoriteHover}
          ]"
...
import { mapMutations, mapGetters, mapActions } from 'vuex';
    ...
    computed: {
      ...
      getId() {
        const { beachId, eventId } = this.data;
        return (beachId && !eventId)
          ? [beachId, 'beach']
          : [eventId, 'event']
      },
      isFav() {
        return this.isFavorite(...this.getId)
      }
    },
		methods: {
      ...
      ...mapActions('favorites', [
        ...
        'removeFavorite',
			...
			updateHeart() {
        this.isFav
          ? this.removeFavorite(this.getId)
          : this.addToFavorite (this.getId);
			},
