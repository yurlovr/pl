			...
			<section class="two-part-layout__card">
				...
				<div class="beach-page__water-temp__content" :class="{ active : modalOpen }">
					<div class="beach-page__water-temp__year-month">
						<button class="slider-weather__month" :class="{ active : activeMonth == -1 }" @click="activeMonth = -1"><span>Год</span></button>
						<button class="slider-weather__month" :class="{ active : activeMonth > -1 }"><span>Месяц</span></button>
					</div>
					<div class="beach-page__water-temp__histogram" v-show="activeMonth == -1">
						<div class="beach-page__water-temp__histogram__item" v-for="(item, i) in monthAvgs" :key="i" @click="activeMonth = i">
							<div class="beach-page__water-temp__histogram__item__bar" :style="{ height: (140 * item / Math.max.apply(null, monthAvgs) + 'px') }">
					...
					<div class="beach-page__water-temp__histogram month" v-if="activeMonth == i" v-for="(month, i) in data" :key="i">
						<div class="beach-page__water-temp__histogram__item" v-for="(item, index) in month" :key="index">
							<div class="beach-page__water-temp__histogram__item__bar" :style="{ height: (140 * item / Math.max.apply(null, month) + 'px') }">
								<div class="beach-page__water-temp__histogram__item__bar__info">
									<span class="slider-weather__slide__temp-number">{{ (item.toFixed(0) > 0 ? '+ ' : '') + (item.toFixed(0) < 0 ? '- ' : '') + item.toFixed(0) }}</span>
									<span class="slider-weather__slide__temp-o"><span>o</span></span>
									<span class="slider-weather__slide__temp-C">C</span>
