<template>
  <div class="beach-event__side-buttons">
    ...
    <transition name="bounce" v-if="data.pos">
      <a target="_blank" :href="`https://yandex.ru/maps/?pt=${data.pos[1]}%2C${data.pos[0]}&text=${data.pos[1]}%2C${data.pos[0]}&z=12&l=map`" v-show="showPave" class="btn-ymaps">
        <img src="~/static/pics/global/svg/ymaps.svg">
      </a>
    </transition>
    <transition name="bounce" v-if="data.pos">
      <a target="_blank" :href="`yandexmaps://maps.yandex.ru/?pt=${data.pos[1]}%2C${data.pos[0]}&text=${data.pos[1]}%2C${data.pos[0]}&z=12&l=map`" v-show="showPave" class="btn-display">
