...
export const actions = {
  async getPageData({ commit, state }, page) {
    ...
    if (!state.allPages || !state.allPages.data || !state.allPages.data.list) return 404;
    const pageData = state.allPages.data.list.find((v) => v.CODE === page);
    if (pageData) {
...
export const getters = {
  data: (state) => {
    ...
    if (state.announcementData && state.announcementData.data) {
      const announcement = state.announcementData.data.list[Math.floor(Math.random() * state.announcementData.data.list.length)]; // getting a random announcement
      if (announcement) {
