...
export const mutations = {
  ...
  SET_NEAR_BEACH: (state, payload) => state.near_beach = payload
}
...
export const actions = {
  async getBeach({commit, state}, id) {
    ...
    commit('SET_NEAR_BEACH', await this.$axios.$get('/hotel/beachList'));
...
export const getters = {
  beachData: (state) => {
    ...
    for (let i = 0; i < state.near_beach.data.list.length; i++) {
      ret.near_beach.push({
        pic: state.near_beach.data.list[i].PICTURE,
        name: state.near_beach.data.list[i].NAME,
        beachDistance: state.near_beach.data.list[i].BEACH_DISTANCE,
        distance: state.near_beach.data.list[i].DISTANCE,
        city: state.near_beach.data.list[i].CITY,
        coordinates: state.near_beach.data.list[i].COORDINATES,
        country: state.near_beach.data.list[i].COUNTRY,
        id: state.near_beach.data.list[i].ID,
        price: state.near_beach.data.list[i].PRICE,
        rating: state.near_beach.data.list[i].RATING,
        url: state.near_beach.data.list[i].URL,
    ...
    if (ret.near_beach.length > 0)
      ret.sections.push({
        title: 'Забронируйте номер рядом',
        hash: 'near-beaches'
      });
